...
URL: https://www.neputa-note.net/entry/2024/11/26/aspnetcore-nginx-docker-conoha-vps
...
> VPSは、物理的には一台のサーバーを複数の利用者で共同利用する点は変わりません。しかし、「利用者があたかも一台の専用サーバーを占有しているかのように仮想的に操作できる」点が違います。このため「バーチャルでプライベートなサーバー」＝VPSと呼ばれるわけです。<br>
> -- [第1回：VPSってなに？ おいしいの？｜ConoHa VPSサポート](https://support.conoha.jp/v/study-01/)より引用

## ConoHa VPSとは？

> ConoHa VPSは初期費用無料、利用料金はサーバー費用のみ。料金タイプはご利用期間に合わせて2種類ご用意しています。さらに、転送量に対する課金は一切ありません。用途に合わせて、はじめたいときに最小限のコストではじめられるVPSです。<br>
> -- [特長｜VPSならConoHa](https://www.conoha.jp/vps/features/)より引用
...
> [!NOTE]
> ポート番号は任意の値を設定（ここでは40022）

3.次に、作成したセキュリティグループをサーバーに設定する

4.サーバーのネットワーク情報のセキュリティグループに、作成した設定を追加する。

5.コントロールパネル左側のメニューより、サーバー → 該当サーバー（ネームタグのリンク）→ ネットワーク情報を選択

6.下段のセキュリティグループの鉛筆アイコンをクリック

7.+アイコンをクリックし、先ほど作成したSSH用のセキュリティグループを追加する

8.Webアプリ公開用に、IPv4v6-Web（80と442）も併せて追加しておく

> [!NOTE]
> pingで疎通テストをするにはIPv4v6-ICMPを追加する
...
> [!NOTE]
> 今回のDebianはopensshがインストール済みだが無い場合は追加する
...
- コントロールパネル→ドメインよりドメインを取得する
- コントロールパネル→DNSより、取得したドメインを追加する
