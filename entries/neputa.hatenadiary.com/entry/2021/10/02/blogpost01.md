...
[f:id:neputa:20260829225745w:plain:alt=アイキャッチ画像]
...
[f:id:neputa:20260829225752w:plain:alt=Rufus1]

続いて「スタート」ボタンで処理を開始する。

当然ながらUSBメディア内のデータはフォーマットされ消失するのでご注意を。

[f:id:neputa:20260829225759w:plain:alt=Rufus2]

「スタート」ボタンをクリックすると、イメージの書き込みモードを問うメッセージが表示されるので、「ISO」を選択されていることを確認し、「OK」。

[f:id:neputa:20260829225807w:plain:alt=Rufus3]

処理が終わるまでしばし待機。

[f:id:neputa:20260829225814w:plain:alt=Rufus4]

「準備完了」が表示されたら「閉じる」ボタンでRufusを終了する。

[f:id:neputa:20260829225823w:plain:alt=Rufus5]
...
[f:id:neputa:20260829225930w:plain:alt=Ubuntu08]

「インストールの種類」を選択する。今回はSSDにプリインストールされているWindowsはいらないので「ディスクを削除してUbuntuをインストール」を選択し、クリーンインストールを行う。

[f:id:neputa:20260829225937w:plain:alt=Ubuntu09]

同画面の「高度な機能」というボタンをクリックすると、このようなウィンドウが表示されるが、今回はいずれも必要ないのでデフォルトの「なし」が選択されているでOK。

[f:id:neputa:20260829225946w:plain:alt=Ubuntu10]

画面右下の「インストール」ボタンを押すとポップアップが立ち上がるので「続ける」を選択する。

[f:id:neputa:20260829225953w:plain:alt=Ubuntu11]

どこに住んでるか聞いてくるので正直に答える。

[f:id:neputa:20260829230001w:plain:alt=Ubuntu12]

ユーザ名やマシン名やパスワードを入力する。

[f:id:neputa:20260829230011w:plain:alt=Ubuntu13]

インストール実行中。しばしご歓談。

[f:id:neputa:20260829230021w:plain:alt=Ubuntu14]

インストール完了。「今すぐ再起動する」を押す。

[f:id:neputa:20260829230028w:plain:alt=Ubuntu15]

起動画面に「Please remove the Installation medium, then press ENTER」と出る。USBのインストールメディアをぶっこ抜いてENTERを叩く。

[f:id:neputa:20260829230036w:plain:alt=Ubuntu16]

はい来た。

[f:id:neputa:20260829230043w:plain:alt=Ubuntu17]
