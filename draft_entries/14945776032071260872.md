...
![アイキャッチ画像](../assets/images/hero__2022__01__add-neovim-context-menu.webp)
...
![スクリーンショット](../assets/images/xJaSRZPo2mXRl7kMIr5O-screenshot.webp)

この状態ではコンテキストメニューの表示が「Edit with Neovim」なのでレジストリエディタで編集する。「あとで編集するなら直接regファイルに書けばよかったじゃん」と思うのだが、日本語がうまく認識されなかったので、この手順とする。

![スクリーンショット](../assets/images/r3aSlZDo2mXRl7vMIr3s-screenshot.webp)

Winキー + Rで「ファイル名を指定して実行」を開き、「regedit」と入力してEnterでレジストリエディタをオープン。

HKEY_CLASSES_ROOT\\\*\shell\nvimの「既定」を右クリック→修正で、データの値を編集する

![スクリーンショット](../assets/images/3dhAM93d1Ko3XlqMIrN3-screenshot.webp)

右クリック、オッケー。

![スクリーンショット](../assets/images/k39w5uxEd18iqdqM23sw-screenshot.webp)

Windows TerminalのPowerShellで開いている、オッケー。

![スクリーンショット](../assets/images/1duyY8dMhba8D1d8Ji1l-screenshot.webp)
